...
import scripts.dax.api_lib.models.*;
import scripts.dax.shared.helpers.InterfaceHelper;
...
public class DaxWalker implements Loggable {
    ...
    public static boolean walkTo(Positionable destination, WalkingCondition walkingCondition) {
        ...
        if (validPaths.size() == 1 && validPaths.get(0).getPathStatus().shouldBackOff()) {
            WaitFor.milliseconds((int) validPaths.get(0).getPathStatus().generateBackoffDuration());
        }
